...
function registerGenerators (Blockly) {
    ...
    const hasArduinoInclude = function (includeText) {
        return Object.keys(Blockly.Arduino.includes_).some(key =>
            Blockly.Arduino.includes_[key].includes(includeText));
    };

    Blockly.Arduino.tubitv2tcs3472_set= function (block) {
        const ledpin = block.getFieldValue('LEDPIN');
        const includes = [];
        if (!hasArduinoInclude('#include <Wire.h>')) {
            includes.push('#include <Wire.h>');
        }
        includes.push('#include "Adafruit_TCS34725.h"');
        Blockly.Arduino.includes_.tubitv2tcs3472_set = includes.join('\n');
        Blockly.Arduino.definitions_.tubitv2tcs3472_set = `float rgb[3] ={0,0,0};\nint tcsLED = ${ledpin};\nAdafruit_TCS34725 tcs = Adafruit_TCS34725(TCS34725_INTEGRATIONTIME_50MS, TCS34725_GAIN_4X);\n`; 
        return`if (tcs.begin()) {\nSerial.println("Found sensor");\n} else {\nSerial.println("No TCS34725 found ... check your connections");\nwhile (1);\n}\nledcAttach(tcsLED, 490, 8);\n`;
    };
