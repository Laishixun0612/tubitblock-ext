...
//----
  ...
  if (is800KHz) {
#endif
    uint32_t top = (F_CPU / 800000);       // 1.25繕s
    uint32_t t0 = top - (F_CPU / 2500000); // 0.4繕s
    uint32_t t1 = top - (F_CPU / 1250000); // 0.8繕s
    ...
    uint32_t top = (F_CPU / 400000);       // 2.5繕s
    uint32_t t0 = top - (F_CPU / 2000000); // 0.5繕s
    uint32_t t1 = top - (F_CPU / 833333);  // 1.2繕s
    SysTick->LOAD = top - 1; // Config SysTick for NeoPixel bit freq
    SysTick->VAL = 0;        // Set to start value
    for (;;) {
      LL_GPIO_SetOutputPin(gpioPort, gpioPin);
